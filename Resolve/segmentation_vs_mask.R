## omparing the segmented objects in NFH to everything else in the tissue
...
## other parameters
options(scipen = 999)
fp0 <- "~/Documents/tmp/2023/202307_piol/data/"
...
# 
# ############################ "SN1 NFH_transversal" ############################
# fp <- paste0("~/Documents/tmp/2023/202307_piol/data/negative_count_matrices/",
#              "SN_Resolve_MC_Images/NFH/negative/")
# lf <- list.files(path = fp, pattern = "*.csv")
# dfs1 <- lapply(lf, function(i){readr::read_csv(file = paste0(fp, i))})
# dfs2 <- lapply(dfs1, function(i){i %>%
#     # dplyr::filter(area < 2500) %>%
...
#   gsub("-", "_", gsub("_labeled_negative-count-matrix.csv", "", lf)), "_SN1_mask"))
# 
# #################################### SN2 #######################################
# 
# ############################# "SN2 NFH_transversal" ############################
...
# 
# ####################### "SN2 NFH_transversal" NEGATIVE #########################
# fp <- paste0("~/Documents/tmp/2023/202307_piol/data/negative_count_matrices/",
#              "SN_Resolve_Images_2/NFH/")
# lf <- list.files(path = fp, pattern = "*.csv")
# dfs1 <- lapply(lf, function(i){readr::read_csv(file = paste0(fp, i))})
# dfs2 <- lapply(dfs1, function(i){i %>%
#     # dplyr::filter(area < 2500) %>%
...
#   gsub("-", "_", gsub("_labeled_non-expanded_negative-count-matrix.csv", "", lf)), "_SN2_mask"))
# 
...
# names(cl1) <- c("SN1_NFH_segmented_objects", "SN2_NFH_segmented_objects",
#                 "SN1_NFH_mask", "SN2_NFH_mask")
...
# resultsNames(dds)
rld1 <- rlogTransformation(dds2)
PCA1 <- plotPCA(rld1, intgroup = "segmentation") +
  theme(plot.title = element_text(hjust = 0.5)) +
  # geom_text_repel(aes(label = colnames(rld)),
  #                 arrow = arrow(length = unit(0.03, "npc"),
  #                 type = "closed", ends = "first"), force = 5) +
  ggtitle(paste0("PCA of SN NFH segmented objects and mask bulk counts (segmentation)"))
PCA2 <- plotPCA(rld1, intgroup = "batch") +
  theme(plot.title = element_text(hjust = 0.5)) +
  # geom_text_repel(aes(label = colnames(rld)),
  #                 arrow = arrow(length = unit(0.03, "npc"),
  #                 type = "closed", ends = "first"), force = 5) +
  ggtitle(paste0("PCA of SN NFH segmented objects and mask bulk counts (batch)"))
...
## size factor plots
barplot(dds2$sizeFactor, main = "Size Factors for samples", col = "lavender",
        cex.axis = 0.7, las = 2, ylab = "Size Factors", cex.names = 0.2) -> b1

############################### DE comparisons #################################

## DE comparisons control segmentation vs mask
res0 <- results(dds2, contrast = c("segmentation", "segmented", "mask"), alpha = 0.05)
merge(as.data.frame(res0), as.data.frame(counts(dds2, normalized = TRUE)),
...
writexl::write_xlsx(x = res_list, path = paste0(
  fp0, "SN_NFH_segmented_objects_and_mask_pseudobulk_DE.xlsx"))
...
res_vol %>%
  ...
  ggtitle("NFH segmented objects vs mask") -> v1

rd0 %>%
  dplyr::slice(1:5) %>%
  dplyr::select(genes, tidyr::contains("NFH")) %>%
  ...
  ggtitle(paste0("Gene counts of top 5 genes")) +
  facet_grid(~genes) -> t1

## save plots
pdf(file = paste0(fp0, "SN_NFH_segmented_objects_and_mask_pseudobulk_plots.pdf"))
PCA1; PCA2; PCA3; b1; v1; t1
dev.off()