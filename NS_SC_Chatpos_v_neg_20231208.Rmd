---
title: "DE analysis of NanoString spinal cord samples Chat+ vs Chat- (control only)"
author: "Theo Killian"
date: "`r Sys.Date()`"
output:
  html_document:
...
This report describes an analysis of Axonseq data provided by Diana Piol
using the [DESeq2](https://bioconductor.org/packages/release/bioc/html/DESeq2.html)
Bioconductor package. This report analyzes the inferred transcriptional changes
between experimental groups and produces diagnostic plots and lists of
differentially expressed genes for each experimental comparison.
...
the conditions of these samples. The metadata and matrix counts from
`r {gsub("_", " ", proj)}` data are subset to contain only
`r {gsub("_", " ", sel_comp)}` sciatic nerve samples. The metadata (or
"colData") for the samples being compared is displayed in an HTML table shown
below:

```{r load_files, warning=FALSE, message=FALSE, echo=FALSE}
## read long format count file
readxl::read_excel(path = paste0(
  "../202311_piol/data/count_matrix_Long_Whole_Dataset.xlsx")) %>% 
...
raw_df %>% 
  ...
  dplyr::filter(region == "SC", ## only contain sciatic nerve samples 
                class == "CTRL") %>% 
  dplyr::mutate_all(as.factor) %>%
  as.data.frame() -> meta_data
rownames(meta_data) <- meta_data$sample
# saveRDS(object =  meta_data, file = paste0("./data/meta_data.rds"))
...
Note: all comparisons are based relative to the *first comparison* as the normative
control
...
Results are saved as *.csv* and *.rds* files in two versions:

1) Unfiltered (with dropouts)
2) Samples within DE comparisons filtered with 66.6% non-zero values
...
saveRDS(object = de_list, file = paste0(
  "./data/", proj, "_", sel_comp, "_DE_res_", Sys.Date(), ".rds"))
writexl::write_xlsx(x = de_list, path =  paste0(
  "./data/", proj, "_", sel_comp, "_DE_res_", Sys.Date(), ".xlsx"))
```
