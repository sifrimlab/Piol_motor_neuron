...
names(chat1) <- c("genes", paste0(gsub("-", "_", gsub(
  "_labeled_non-expanded_count-matrix_with-area.csv", "", lf1)), "_SN1"))
...
names(chat2) <- c("genes", paste0(gsub("-", "_", gsub(
  "_labeled_non-expanded_count-matrix_with-area.csv", "", lf2)), "_SN2"))
...
all_Chat %>%
  ...
  geom_point() +
  geom_hline(yintercept = mean(all_Chat$all_counts), color = "red",
             linetype = "dashed") +
  theme(axis.text.x = element_text(angle = 75, vjust = 1, hjust = 1),
        legend.position = "none") +
  ylab("Total gene counts per sample") +
  ggtitle(paste0("Total non-normalized pseudobulk counts in SN Chat")) -> p1
...
names(nfh1) <- c("genes", paste0(gsub("-", "_", gsub(
  "_labeled_non-expanded_count-matrix_with-area.csv", "", lf3)), "_SN1"))
...
names(nfh2) <- c("genes", paste0(gsub("-", "_", gsub(
  "_labeled_non-expanded_count-matrix_with-area.csv", "", lf4)), "_SN2"))
...
all_NFH %>%
  ggplot(aes(x = control, y = all_counts, color = control)) +
  geom_point() +
  geom_hline(yintercept = mean(all_NFH$all_counts), color = "red",
             linetype = "dashed") +
  theme(axis.text.x = element_text(angle = 75, vjust = 1, hjust = 1),
        legend.position = "none") +
  ylab("Total gene counts per sample") +
  ggtitle("Total non-normalized pseudobulk counts in SN NFH") -> p2
...
data.frame(colSums(total_counts)) %>%
  ...
  dplyr::mutate(
    ...
    control = forcats::fct_relevel(control, c("CtrlFUSH+CtrlTDP43", "FUSH",
                                              "TDP43", "CtrlSOD1", "SOD1"))) %>%
      dplyr::filter(all_counts < 300) -> all_Chat_seg

all_Chat_seg %>%
  dplyr::filter(all_counts < 300) %>%
  ggplot(aes(x = control, y = log10(all_counts), color = control, fill = control)) +
  geom_jitter(size = 0.5, alpha = 0.5, color = "black") +
  geom_violin() +
  geom_hline(yintercept = log10(mean(all_Chat_seg$all_counts, na.rm = TRUE)),
             color = "red", linetype = "dashed") +
  theme(axis.text.x = element_text(angle = 75, vjust = 1, hjust = 1),
        legend.position = "none") +
  ylab("log10 total gene counts per segment") +
  ggtitle("Total non-normalized pseudobulk counts per segment in SN Chat") -> p3
...
data.frame(colSums(total_counts)) %>%
  ...
  dplyr::mutate(
    ...
    control = forcats::fct_relevel(control, c("CtrlFUSH+CtrlTDP43", "FUSH",
                                              "TDP43", "CtrlSOD1", "SOD1"))) %>%
  dplyr::filter(all_counts < 300) -> all_NFH_seg

all_NFH_seg %>%
  ggplot(aes(x = control, y = log10(all_counts), color = control, fill = control)) +
  geom_jitter(size = 0.5, alpha = 0.5, color = "black") +
  geom_violin() +
  geom_hline(yintercept = log10(mean(all_NFH_seg$all_counts, na.rm = TRUE)),
             color = "red", linetype = "dashed") +
  theme(axis.text.x = element_text(angle = 75, vjust = 1, hjust = 1),
        legend.position = "none") +
  ylab("log10 total gene counts per segment") +
  ggtitle("Total non-normalized pseudobulk counts per segment in SN NFH") -> p4

## save pseudobulk_gene_counts
pdf(file = paste0(out, "SN_NFH_and_Chat_segment_gene_counts.pdf"))
p1; p2; p3; p4
dev.off()
