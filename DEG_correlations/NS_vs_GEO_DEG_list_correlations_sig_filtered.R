...
## heatmaps to establishing uniqueness in Piol, et al DE results.

## NOTE: DE genes are correlated by mapping mouse ensembl IDs across datasets,
## and correlating the log fold change. in the case of the Yadav, et al and 
## Gautier et al, datasets, the human genes were mapped to mouse orthologs, 
## using the biomaRt R package
...
rp <- "~/Documents/tmp/dacruz/202406_piol/data/"
...
readRDS("~/Documents/tmp/dacruz/202402_piol/data/Gauthier_data/GSE228778_motoneuron_DEGs.rds") %>%
  dplyr::rename(genes = names(.)[1]) %>%
  dplyr::mutate(Levine_GSE190442_gene_rank = row_number()) %>%
...
#   as.data.frame() -> DE_Nanostring_sig_genes_old
# View(DE_Nanostring_sig_genes_old)

readxl::read_excel("~/Documents/tmp/dacruz/202406_piol/data/NanoString_spinal_cord_CTRL_DE_res_2024-06-04.xlsx") %>%
...
## Get Gabriele 10X Visium results
...
list(`Piol`      = DE_Nanostring_ensembl$ensembl_gene_id,
  ...
  display_venn(
    main = paste0("Venn diagram of ", gsub("_", " ", filter_var), " Chat+/CHAT+ vs Chat-/CHAT- DE genes mapped",
...
## only LFC > 0 = enriched in axons
# DE_Nanostring_sig_genes_unique %>%
#   dplyr::filter(log2FoldChange > 0) -> chat_pos_enriched
...
write_xlsx(x = l0, path = paste0(rp, "all_", filter_var, "_Chatpos_vs_Chatneg_unfiltered_DEG_lists_final_correct.xlsx"))

### create list of all DEG lists AFTER DEDUPLICATION
# list(DE_Nanostring_sig_genes_unique %>% dplyr::select(-contains("rank")), ## piol unique
#      readRDS(file = paste0(fp, "DE_results/RDS/DE_Nanostring_sig_genes.rds")) %>%
#        dplyr::select(-contains("rank")), ## piol
#      Gautier_GSE228778_DE_RES %>% dplyr::select(-contains("rank")), ## gautier
#      Levine_GSE190442_DE_RES %>% dplyr::select(-contains("rank")), ## yadav
#      readRDS("~/Documents/tmp/dacruz/202403_piol/data/new_DE/GSE161621_Blum_author_ann_DE_final.rds") %>% ## new blum DE
#        dplyr::rename(genes = names(.)[1]) %>%
#        # dplyr::left_join(mouse_gene_ids, by = "genes") %>%
#        # dplyr::select(genes, ensembl_gene_id, external_synonym, everything()) %>%
#        # dplyr::select(-contains("rank")) %>% 
#        as.data.frame(),
#      readRDS("~/Documents/tmp/dacruz/202403_piol/data/new_DE/GSE167597_Alkaslasi_author_ann_v2_DE.rds") %>% ## new Alkaslasi DE
#        dplyr::rename(genes = names(.)[1]) %>%
#        # dplyr::left_join(mouse_gene_ids, by = "genes") %>%
#        # dplyr::select(genes, ensembl_gene_id, external_synonym, everything()) %>%
#        # dplyr::select(-contains("rank")) %>%
...
# 
# ## save Excel
# saveRDS(object = l0, file = paste0(rp, "all_Chatpos_vs_Chatneg_DE_gene_lists_final.rds"))
# write_xlsx(x = l0, path = paste0(rp, "all_Chatpos_vs_Chatneg_DE_gene_lists_final.xlsx"))
...
write_xlsx(x = de_ns,
           path = paste0(rp, "all_", filter_var, "_Chatpos_vs_Chatneg_deduplicated_LFC_final_correct.xlsx"))
...
ggsave(filename = paste0("~/Documents/tmp/dacruz/202406_piol/", filter_var, "correlation_Piol_v_Blum.png"),
       device = "png", plot = p1)
ggsave(filename = paste0("~/Documents/tmp/dacruz/202406_piol/", filter_var, "correlation_Piol_v_Alkaslasi.png"),
       device = "png", plot = p2)
ggsave(filename = paste0("~/Documents/tmp/dacruz/202406_piol/", filter_var, "correlation_Piol_v_Yadav.png"),
       device = "png", plot = p3)
ggsave(filename = paste0("~/Documents/tmp/dacruz/202406_piol/", filter_var, "correlation_Piol_v_Gautier.png"),
       device = "png", plot = p4)

## print pdf of correlations
# pdf(file = "~/Documents/tmp/dacruz/202403_piol/plots/DE_LFC_correlations_final_correct.pdf")
...
###################### print all plots
# de_ns %>%
#   dplyr::select(ensembl_gene_id, contains("_lfc")) %>%
#   dplyr::arrange(ensembl_gene_id) %>%
#   tibble::column_to_rownames(var = "ensembl_gene_id") -> corr_mat
# names(corr_mat) <- gsub("_lfc", "", names(corr_mat))
# corr_mat[is.na(corr_mat)] <- 0
# 
# pdf(file = paste0("~/Documents/tmp/dacruz/202403_piol/plots/NanoString_GEO_LFC_cormat_final_correct.pdf"))
# M <- cor(corr_mat, method = "spearman")
# corrplot(M, method = 'color', title = "Spearman correlations of Log2 FC",
#          mar=c(0,0,2,0))
# corrplot(M, method = 'number', title = "Spearman correlations of Log2 FC",
#          mar=c(0,0,2,0))
# M <- cor(corr_mat, method = "pearson")
# corrplot(M, method = 'color', title = "Pearson correlations of Log2 FC",
#          mar=c(0,0,2,0))
# corrplot(M, method = 'number', title = "Pearson correlations of Log2 FC",
#          mar=c(0,0,2,0))
# dev.off()