...
fp0 <- "~/Documents/tmp/2023/202308_piol/plots/"
...
for (i in 1:length(unique(all_chat$genes))) {
  all_chat %>%
    dplyr::filter(genes == sort(unique(all_chat$genes))[i]) %>%
    ggplot(aes(x = control, y = counts, color = control)) +
    geom_point() +
    ggtitle(paste0(unique(all_chat$genes)[i], " non-normalized pseudobulk counts")) +
    # geom_hline(aes(yintercept = unique(all_chat$mean_counts)[i]), color = "red",
    #            linetype = "dashed") +
...
pdf(file = paste0(fp0, "SN_NFH_1500_Chat_pos_neg_pseudobulk_gene_counts.pdf"))
print(fgl1)
dev.off()
