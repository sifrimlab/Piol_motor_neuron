---
title: "DE analysis of NanoString GeoMx Chat+ vs Chat- (control only) spinal cord samples"
author: "Theo Killian"
date: "`r Sys.Date()`"
output:
  html_document:
...
read.table(file = paste0("./data/Nanostring_metadata_table.tsv"), sep = "\t",
           header = TRUE) %>%
  dplyr::filter(region == "SC", ## only contain spinal cord samples
                class == "CTRL") %>%
...
read.table(file = paste0("./data/Nanostring_processed_count_matrix.tsv"), sep = "\t",
           header = TRUE) %>%
  dplyr::select(meta_data$sample) %>% ## only contain spinal cord samples
  as.data.frame() -> cts
